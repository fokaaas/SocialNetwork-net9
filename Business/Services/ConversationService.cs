...
public class ConversationService : IConversationService
{
    ...
    {
        var conversation = await _unitOfWork.ConversationRepository.GetByIdAsync(conversationId);
        if (conversation is null) throw new NotFoundException("Conversation not found");

        if (!conversation.IsGroup) throw new BadRequestException("You can't update private chat");

        var participant =
            await _unitOfWork.ConversationParticipantRepository.GetByIdAsync(conversationId, currentUserId);
        ...
        foreach (var participantId in model.ParticipantIdsToRemove)
        {
            ...
            if (participantToRemove is not null) conversation.Participants.Remove(participantToRemove);
        }
    ...
    public async Task UpdateParticipantAsync(int conversationId, int currentUserId, int userId,
        ConversationParticipantUpdateModel model)
    {
        var conversation = await _unitOfWork.ConversationRepository.GetByIdAsync(conversationId);
        if (conversation is null) throw new NotFoundException("Conversation not found");
        ...
        if (participantToUpdate is null) throw new NotFoundException("Participant not found");
