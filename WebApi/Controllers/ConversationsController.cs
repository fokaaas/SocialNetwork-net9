...
public class ConversationsController : ControllerBase
{
    ...
    public async Task<ActionResult> UpdateParticipant(int conversationId, int userId,
        [FromBody] ConversationParticipantUpdateModel participantModel)
    {
        var currentUserId = HttpContext.Items["UserId"].ToString();
        await _conversationService.UpdateParticipantAsync(conversationId, Convert.ToInt32(currentUserId), userId,
            participantModel);
        return Ok();
    }
}